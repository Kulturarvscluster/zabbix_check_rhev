...
scp -p $SCRIPT_DIR/userparameter_ovirt-3.conf.properties $OVIRT:/etc/zabbix/zabbix_agentd.d/userparameter_ovirt.conf
...
grep zabbix ~/.netrc | xargs -r -I{} ssh $OVIRT " ( cat /var/lib/zabbix/.netrc | grep '{}') || (echo '{}' | cat > /var/lib/zabbix/.netrc ) ; chmod go-rwx /var/lib/zabbix/.netrc; chown zabbix:zabbix /var/lib/zabbix/.netrc"
...
#[root@pc543 ~]# zabbix_get -s kac-man-002.kac.sblokalnet --tls-connect psk --tls-psk-identity YAKPSK --tls-psk-file ~/yakpsk.psk -k 'system.hostname'
